...
- `state.json` 台账落盘后收紧为 0600:其中 setx_keys 记录被覆盖前的旧
  环境变量值(往往是一把真实密钥),此前是全链路唯一以默认权限(0644)
  写密钥的位置
- 安装子进程环境剔除凭证变量:`run_command` 的三个 Popen 分支统一传入
  白名单 env(剔除名称含 API_KEY/AUTH_TOKEN/SECRET 的变量)。主循环
  "逐工具先安装后配置"意味着前面工具写入 os.environ 的密钥会被后面
  工具的第三方安装脚本继承,现在该泄漏路径被阻断
- 模型 ID 出口字符集校验(`get_model_ids`):ID 会被插值进 .cmd 启动器、
  TOML 表头与 sh 脚本,集合 `[A-Za-z0-9._/-]` 之外的 ID 一律丢弃并告警
  ——对"远程目录/发现 API 内容被污染"的纵深防御(现有全部目录 ID 合规,
  含 deepseek/ 厂商前缀形式)
- API Key 入口字符集校验:含引号/空白/`$`/反引号/`;`/`|`/`&`/`%` 等
  元字符的"Key"在进入流程时拒绝(env 文件是唯一不做格式转义的密钥落盘
  点,入口拒绝优于下游转义——后者需随各工具格式演进重跑 E2E)
- `setup.bat` 标题与横幅品牌残留修正:"腾讯云 Token Plan" → "TokenHub"
  (2.6.0 品牌统一时的漏网之鱼,横幅列宽按 CJK 双宽重新对齐)
- 测试隔离补强:conftest 新增 autouse fixture,每条用例前重置 adapters
  的模块级缓存(远程目录/后付费发现结果),消除测试顺序依赖(已用全量
  逆序执行验证);新增 5 项安全回归测试(state 权限/子进程 env 剥离/
  模型 ID 校验/Key 字符集门/流程拒绝路径)

审计结论:模块依赖为无环 DAG(infrastructure→domain→adapters→flows→cli,
import-linter 契约固化);无 eval/exec/pickle 危险原语;全部 subprocess
列表参数无注入面;出站 URL 全固定且 TLS 默认校验;npm 全线
`--ignore-scripts` + 精确版本 pin;密钥文件全量 0600;远程脚本下载-
落盘-确认执行无 TOCTOU。遗留已知边界(不阻塞):远程脚本哈希仅展示
(上游不发布固定哈希,建议后续引入滚动清单)、npm integrity 安装期未
强制(CI 周度门禁覆盖)、写入非原子(崩溃可留半截文件,均有备份可恢复)。
