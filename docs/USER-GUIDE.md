...
本工具是命令行配置安装器,用于将腾讯云大模型 API 的接入参数(服务端点、API Key、模型列表)写入 Claude Code、Codex 等 14 个 AI 编程工具的配置文件。
...
**步骤四:选择工具**。列出 14 个工具,直接回车配置全部;输入编号(逗号或空格分隔,如 `3,7,13`)仅配置指定工具。

确认后自动执行:未安装的工具自动安装,配置自动写入,完成后逐项显示结果。

桌面应用(WorkBuddy 除外)不支持自动写入,安装器会输出需要手动填写的字段(端点地址、API Key),在应用设置中填写一次即可。
...
支持的 14 个工具:Hermes Agent、CodeBuddy Code、Claude Code、OpenCode、OpenClaw、DeepSeek Harness、Codex CLI、Kilo CLI、Kilo Code、Cline、WorkBuddy、AutoClaw、QClaw、CoPaw。
